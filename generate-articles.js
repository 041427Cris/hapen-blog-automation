...
Cada artigo deve ter NO MÁXIMO 400 palavras — isso é um limite rígido, não uma sugestão. Seja direto, sem enrolação nem repetição.
HTML simples (h2, h3, p, ul/li — sem head/body/html), pronto para colar no editor do WordPress.
...
exports.handler = async (event) => {
  ...
  try {
    const results = await Promise.allSettled(ANGLES.map((angle) => callClaude(apiKey, imovel, angle)));
    const articles = results.filter((r) => r.status === "fulfilled").map((r) => r.value);
    const errors = results.filter((r) => r.status === "rejected").map((r) => r.reason?.message || "erro desconhecido");

    if (!articles.length) {
      return { statusCode: 500, body: JSON.stringify({ error: "Nenhum artigo foi gerado: " + errors.join(" | ") }) };
    }

    return { statusCode: 200, body: JSON.stringify({ articles, partialErrors: errors.length ? errors : undefined }) };
  } catch (err) {
