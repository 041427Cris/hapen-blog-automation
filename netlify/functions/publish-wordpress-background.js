...
async function uploadFeaturedImage(page, imageUrl) {
  // Baixa a imagem da galeria do imóvel (ou a enviada pelo usuário) e faz
  // upload pelo modal de mídia do WordPress.
  ...
  if (fileChooser) {
    // filechooser precisa de um caminho em disco; gravamos o buffer em /tmp
    const fs = require("fs");
...
async function publishOne(page, baseUrl, article, imovel) {
  ...
  if (imovel.gallery && imovel.gallery.length) {
    await uploadFeaturedImage(page, imovel.coverImage || imovel.gallery[0]).catch((e) => {
